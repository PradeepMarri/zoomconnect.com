...
func SearchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["searchEmail"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("searchEmail=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/api/rest/v1/account/user%s", cfg.BaseURL, queryString)
		...
		var result models.WebServiceUsers
...
func CreateSearchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_rest_v1_account_user",
		mcp.WithDescription("search"),
		mcp.WithString("searchEmail", mcp.Required(), mcp.Description("search by email address")),
	)
