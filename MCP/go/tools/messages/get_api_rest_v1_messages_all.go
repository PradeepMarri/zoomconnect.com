...
func Get_api_rest_v1_messages_allHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["fromDateTimeReceived"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("fromDateTimeReceived=%v", val))
		}
		if val, ok := args["toDateTimeReceived"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("toDateTimeReceived=%v", val))
		}
		if val, ok := args["fromNumber"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("fromNumber=%v", val))
		}
		...
		if val, ok := args["read"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("read=%v", val))
		}
		if val, ok := args["repliesToMessageId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("repliesToMessageId=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/api/rest/v1/messages/all%s", cfg.BaseURL, queryString)
		...
		var result models.WebServiceMessages
...
func CreateGet_api_rest_v1_messages_allTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_rest_v1_messages_all",
		mcp.WithDescription("all"),
		mcp.WithNumber("pageSize", mcp.Description("number of elements to return at a time")),
		mcp.WithNumber("page", mcp.Description("page number")),
		mcp.WithString("type", mcp.Description("filter by message type")),
		...
		mcp.WithString("fromDateTimeReceived", mcp.Description("date format: yyyyMMdd")),
		mcp.WithString("toDateTimeReceived", mcp.Description("date format: yyyyMMdd")),
		mcp.WithString("fromNumber", mcp.Description("phone number the message was sent from")),
		...
		mcp.WithBoolean("read", mcp.Description("return only read / unread messages (inbox messages only)")),
		mcp.WithString("repliesToMessageId", mcp.Description("return only inbox messages which are a reply to the message with the given message id")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_api_rest_v1_messages_allHandler(cfg),
	}
}
